...
v1-gpu-phase2 BS=256 + num_stages=3: Memory latency hiding optimization.
"""
...
def prefix_sum(x: torch.Tensor) -> torch.Tensor:
    ...
    if n_elements <= BLOCK_SIZE:
        ...
        _prefix_sum_single_block_kernel[(1,)](x, out, n_elements, BS, num_stages=3)
        return out
    out = torch.empty_like(x)
    ...
    _prefix_sum_phase1_kernel[(n_blocks,)](x, block_pos_counts, block_sums_even, block_sums_odd, n_elements, BLOCK_SIZE, num_stages=3)
    ...
    _prefix_sum_phase3_kernel[(n_blocks,)](x, out, block_pos_prefix, block_sum_prefix, n_elements, BLOCK_SIZE, num_stages=3)
    return out
