...
v1-gpu-phase2 BS=256 + num_stages=4: Aggressive memory latency hiding
"""
...
def prefix_sum(x: torch.Tensor) -> torch.Tensor:
    ...
    if n_elements <= BLOCK_SIZE:
        ...
        _prefix_sum_single_block_kernel[(1,)](x, out, n_elements, BS, num_stages=4)
        return out
    out = torch.empty_like(x)
    ...
    _prefix_sum_phase1_kernel[(n_blocks,)](x, block_pos_counts, block_sums_even, block_sums_odd, n_elements, BLOCK_SIZE, num_stages=4)
    ...
    _prefix_sum_phase3_kernel[(n_blocks,)](x, out, block_pos_prefix, block_sum_prefix, n_elements, BLOCK_SIZE, num_stages=4)
    return out
