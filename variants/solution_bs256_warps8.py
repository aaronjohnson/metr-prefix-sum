...
Optimization: num_warps=8 (increased parallelism)
- Default is typically 4 warps per block
- 8 warps = 256 threads, may improve occupancy
"""
...
def prefix_sum(x: torch.Tensor) -> torch.Tensor:
    ...
    if n_elements <= BLOCK_SIZE:
        ...
        _prefix_sum_single_block_kernel[(1,)](x, out, n_elements, BS, num_warps=8)
        return out
    out = torch.empty_like(x)
    ...
    _prefix_sum_phase1_kernel[(n_blocks,)](x, block_pos_counts, block_sums_even, block_sums_odd, n_elements, BLOCK_SIZE, num_warps=8)
    ...
    _prefix_sum_phase3_kernel[(n_blocks,)](x, out, block_pos_prefix, block_sum_prefix, n_elements, BLOCK_SIZE, num_warps=8)
    return out
