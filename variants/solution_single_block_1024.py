...
Optimization: Extended single-block threshold to 1024
- Avoids multi-phase overhead for inputs up to 1024 elements
- Single kernel launch vs 3 kernels + torch ops
"""
...
def prefix_sum(x: torch.Tensor) -> torch.Tensor:
    ...
    # Extended single-block path: handles up to 1024 elements in one kernel
    if n_elements <= 1024:
        out = torch.empty_like(x)
        BS = triton.next_power_of_2(n_elements)
        _prefix_sum_single_block_kernel[(1,)](x, out, n_elements, BS)
        return out

    # Multi-block path for larger inputs
    BLOCK_SIZE = 256
    out = torch.empty_like(x)
