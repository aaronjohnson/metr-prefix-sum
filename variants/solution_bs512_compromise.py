...
Optimization: BS=512 with extended single-block threshold
- BS=512 was second-best in our tests
- Extend single-block to 512 to use fast path more often
- Fewer blocks = less Phase 2 overhead
"""
...
def prefix_sum(x: torch.Tensor) -> torch.Tensor:
    ...
    # Extended single-block: up to 512 elements
    if n_elements <= 512:
